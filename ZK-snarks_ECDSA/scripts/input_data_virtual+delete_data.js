...
export const createMerkleTree = async (assetId, buyerAddress, treeHeight) => { // Added treeHeight
    ...
    async function getMerkleProof(smt, key) {
        const F = smt.F;
        const keyScalar = F.e(BigInt(key));  // Đảm bảo key là BigInt
        ...
        while (path.length < treeHeight) { // treeHeight từ file circom là 20
            path.push("0");
        }
    ...
    const initialRoot = poseidon.F.toString(smt.root);
    console.log(`[JS DEBUG] Merkle Root after inserting primary asset (${assetId}): ${initialRoot}`);

    // Điền thêm các giá trị ngẫu nhiên vào cây để minh họa/kiểm tra
    const dummyKeys = []; // Store dummy keys to potentially delete later
    for (let i = 1; i <= 99; i++) { // Start from 1, so keys don't conflict with assetId easily
        const dummyAssetId = BigInt(assetId) + BigInt(i); // <-- SỬA LỖI TẠI ĐÂY
        const randomValue = getRandomValue();
        await smt.insert(dummyAssetId, BigInt(randomValue));
        dummyKeys.push(dummyAssetId);
    }
    const rootAfterDummies = poseidon.F.toString(smt.root);
    console.log(`[JS DEBUG] Merkle Root after inserting all 99 dummy assets: ${rootAfterDummies}`);

    // --- BƯỚC MỚI: XÓA MỘT NÚT GIẢ ---
    const keyToDelete = dummyKeys[50]; // Chọn nút giả thứ 50 để xóa
    console.log(`[JS DEBUG] Attempting to delete dummy key: ${keyToDelete}`);
    // Để xóa, bạn cần oldKey và oldValue của node đó
    const { found, foundValue } = await smt.find(keyToDelete);
    if (found) {
        await smt.delete(keyToDelete, foundValue);
        const rootAfterDeletion = poseidon.F.toString(smt.root);
        console.log(`[JS DEBUG] Merkle Root AFTER DELETION of ${keyToDelete}: ${rootAfterDeletion}`);
        if (rootAfterDeletion === rootAfterDummies) {
            console.warn("[WARNING] Merkle Root DID NOT change after deletion. This might indicate an issue.");
        } else {
            console.log("[INFO] Merkle Root successfully changed after deletion.");
        }
    } else {
        console.warn(`[WARNING] Dummy key ${keyToDelete} not found for deletion. Skipping deletion test.`);
    }
    // --- KẾT THÚC BƯỚC XÓA ---
    ...
    const proofKey = await getMerkleProof(smt, assetId.toString());
    console.timeEnd("Create Proof");

    console.timeEnd("createMerkleTree");
    return {
        root: poseidon.F.toString(smt.root), // Root cuối cùng của SMT sau tất cả các thao tác
        proof: proofKey,
...
async function generateCircomInput() {
    ...
    const outputFile = path.join(outputDir, 'input2.json');
    ...
    console.log(`    - "root" trong public.json (từ Circom) Cần giống với "Merkle Root AFTER DELETION" (từ JS)`);
