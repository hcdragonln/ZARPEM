...
export const createMerkleTree = async (assetId, buyerAddress) => {
    ...
    async function getMerkleProof(smt, key) {
        ...
        while (path.length < 20) { // treeHeight từ file circom là 20
            path.push("0");
        }
    ...
    for (let i = assetId + 1; i <= assetId + 99; i++) {
        const randomValue = getRandomValue(); // getRandomValue trả về chuỗi thập phân
        // Key và Value cho SMT.insert() đều phải là BigInt
...
async function generateCircomInput() {
    ...
    const { root, proof, computedValue } = await createMerkleTree(assetID, buyerAddress);
    ...
    const outputFile = path.join(outputDir, 'input3.json');
